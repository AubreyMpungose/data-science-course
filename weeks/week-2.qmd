...
Data come from various sources: web, local computer, API, databases, etc. In most cases, we would want to import tabular data as a dataframe. Tabular data is normally stored as comma-separated values (.csv) or excel (.xlsx, .xls, etc). It is important to keep not of the extension of the file before loading it.

Next, you will need to know the directory in which your data file is stored. It is very important to know file paths of your computer and this is the foundational skill you need to master in any data science career. We will return on this later. 
...
In this section, we will use the `ggplot2` package within `tidyverse` to learn the foundations of data visualisation. 'ggplot' stands for *grammar of graphics*. 
...
We can see that `gapminder` has `r ncol(gapminder)` and `r nrow(gapminder)`. 
...
2. `ggplot(data = gapminder, aes(x = gdpPercap, y = lifeExp))`: we are adding *mapping aesthetics* or *aesthetics*, allocating x, y axis. There are other *aesthetics* that we can add, such as *size, colour, shape, group, etc*. We will use these later in this section.
...
`ggplot()` is a very rich library for visualisation. There are various ways we can improve our plot. Let us create a basic scatter plot:
...
### 3.5.1 Transforming axis scales
...
There are other external themes packages and the list is non-exhaustive.
